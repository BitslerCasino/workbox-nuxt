...
function getOptions(moduleOptions) {
  ...
  const defaults = { filename: 'sw.js' }
  Object.assign(defaults, {
    autoRegister: true,
    routerBase,
    publicPath,
    swSrc: path.resolve(this.options.buildDir, 'sw.template.js'),
    swDest: path.resolve(this.options.srcDir, this.options.dir.static || 'static', defaults.filename),
    directoryIndex: '/',
...
function addTemplates(options) {
  ...
  if (options.autoRegister) {
    const swURL = `${options.routerBase}/${options.swURL || options.filename}`
    this.addPlugin({
